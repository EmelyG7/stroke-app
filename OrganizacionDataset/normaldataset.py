...
# BASE_PATH = Path("C:/Users/Coshita/Downloads/stroke datos de entrenamiento/DWI/DWI Clase Stroke Imagenes")
# DESTINATION_FOLDER = BASE_PATH / "stroke_consolidado"
#
# # Crear carpeta de destino si no existe
# DESTINATION_FOLDER.mkdir(exist_ok=True)
#
# # Patrones base de las subcarpetas (sin los números finales)
# SUBCARPETAS_BASE = [
#     "dDWI_og_SENSE_",
#     "DWI_og_",
#     "eReg__DWI_og_SENSE_",
...
# ]
#
# # Procesar pacientes del 1 al 37
# for paciente_num in range(1, 38):
#     paciente_folder = BASE_PATH / f"paciente{paciente_num}"
#
#     if not paciente_folder.exists():
#         print(f"Advertencia: No se encontró la carpeta {paciente_folder}")
#         continue
#
#     # Buscar subcarpetas que coincidan con los patrones base
#     for subcarpeta_base in SUBCARPETAS_BASE:
#         # Encontrar todas las subcarpetas que comiencen con el patrón base
#         subcarpetas = [d for d in paciente_folder.iterdir()
#                        if d.is_dir() and d.name.startswith(subcarpeta_base)]
#
#         for subcarpeta in subcarpetas:
#             # Procesar cada archivo en la subcarpeta
#             for file_path in subcarpeta.glob("IM*.jpg"):  # Solo archivos JPG
#                 if file_path.is_file():
#                     # Generar nuevo nombre manteniendo el formato original
#                     new_name = file_path.name.replace("IM", f"strokecase{paciente_num:03d}_dwi_slice", 1)
#                     new_path = DESTINATION_FOLDER / new_name
#
#                     # Copiar el archivo con el nuevo nombre
#                     shutil.copy2(file_path, new_path)
#                     print(f"Copiado: {file_path} -> {new_name}")
#
# print("Proceso completado. Todas las imágenes han sido copiadas y renombradas.")
...
# DESTINATION_FOLDER.mkdir(exist_ok=True)
#
# # Patrones base de las subcarpetas
# SUBCARPETAS_BASE = [
#     "dDWI_og_SENSE_",
#     "DWI_og_",
#     "eReg__DWI_og_SENSE_",
...
#
# # Procesar pacientes del 1 al 37
# for paciente_num in range(1, 14):
#     paciente_folder = BASE_PATH / f"paciente{paciente_num}"
#
#     if not paciente_folder.exists():
#         print(f"Advertencia: No se encontró la carpeta {paciente_folder}")
...
#         for subcarpeta in subcarpetas:
#             # Procesar cada archivo en la subcarpeta
#             for file_path in subcarpeta.glob("*.jpg"):  # Todos los archivos JPG
#                 if file_path.is_file():
#                     # Verificar si el nombre ya está en formato correcto
#                     if file_path.name.startswith(f"strokecase{paciente_num:03d}_"):
#                         # Si ya tiene el formato correcto, copiar tal cual
#                         new_name = file_path.name
#                     elif file_path.name.startswith("IM"):
#                         # Renombrar archivos que empiezan con IM
#                         new_name = file_path.name.replace("IM", f"strokecase{paciente_num:03d}_dwi_slice", 1)
#                     else:
#                         # Para otros formatos, crear nombre consistente
#                         slice_num = file_path.stem.split('_')[-1] if '_' in file_path.stem else file_path.stem
#                         new_name = f"strokecase{paciente_num:03d}_dwi_slice{slice_num}.jpg"
#
#                     new_path = DESTINATION_FOLDER / new_name
#
