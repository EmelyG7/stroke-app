...
# Modelo EfficientNetB3 con atención
def create_efficient_net_model(input_shape=(240, 240, 3)):
...
def train_efficient_net_model(x_train_paths, y_train, x_val_paths, y_val, fold):
    ...
    callbacks = [
        ...
        ModelCheckpoint(f"best_efficientnetb3_fold_{fold}.keras", monitor="val_auc", mode='max', save_best_only=True,
                        verbose=1)
    ]
...
def visualize_results(history, results, y_val, y_pred_prob, fold, model_name="EfficientNetB3"):
...
if __name__ == "__main__":
    data_dir = "C:/Users/Coshita/Downloads/stroke datos de entrenamiento/DWI/dataset_dwi_stroke"  # Ruta en Kaggle
    ...
    for train_idx, val_idx in sgkf.split(all_paths, all_labels, patient_groups):
        ...
        model, history = train_efficient_net_model(x_train_paths, y_train, x_val_paths, y_val, fold)
        metrics, predictions = evaluate_model_with_optimal_threshold(model, x_val_paths, y_val)

        print(f"Fold {fold} - EfficientNetB3:")
    ...
    print("\nResultados Promedio EfficientNetB3:")
    for key, value in avg_metrics.items():
        print(f"{key.capitalize()}: {value:.4f}")

    # Evaluación en conjunto de test (20% del dataset como test)
    ...
    best_model_path = f"best_efficientnetb3_fold_{best_fold}.keras"
