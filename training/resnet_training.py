...
def train_resnet_model(x_train_paths, y_train, x_val_paths, y_val, fold):
    ...
    model.compile(
        ...
        metrics=['accuracy', AUC(name='auc'), F1Score(threshold=0.5)]
    )
    callbacks = [
        ...
        ModelCheckpoint(f"best_resnet50_fold_{fold}.keras", monitor="val_auc", mode='max', save_best_only=True,
                        verbose=1)
    ]
...
def visualize_results(history, results, y_val, y_pred_prob, fold, model_name="ResNet50"):
...
if __name__ == "__main__":
    data_dir = "C:/Users/Coshita/Downloads/stroke datos de entrenamiento/DWI/dataset_dwi_stroke"  # Cambia esto a la ruta de tus datos
    ...
    for train_idx, val_idx in sgkf.split(all_paths, all_labels, patient_groups):
        ...
        model, history = train_resnet_model(x_train_paths, y_train, x_val_paths, y_val, fold)
        metrics, predictions = evaluate_model_with_optimal_threshold(model, x_val_paths, y_val)

        print(f"Fold {fold} - ResNet50:")
    ...
    best_model_path = f"best_resnet50_fold_{best_fold}.keras"
